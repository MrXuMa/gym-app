-- Dedicated response_summaries array in coach_context (longer memory than recent_sessions).
...
BEGIN
  ...
  -- One-time migration path from legacy coach_notes
...
COMMENT ON FUNCTION public.record_coach_advice_in_context(uuid) IS
  'Appends a response summary to coach_context.response_summaries (max 25) for long-term LLM memory.';
