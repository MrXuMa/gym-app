-- Profile lifting experience level for coach template generation.

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS lifting_level text;

ALTER TABLE public.profiles
  DROP CONSTRAINT IF EXISTS profiles_lifting_level_check;

ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_lifting_level_check
  CHECK (
    lifting_level IS NULL
    OR lifting_level = ANY (ARRAY['new', 'beginner', 'intermediate', 'advanced', 'veteran'])
  );
...
CREATE OR REPLACE FUNCTION public.update_profile_lifting_level(p_level text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $function$
DECLARE
  normalized text := lower(trim(p_level));
BEGIN
  ...
  IF normalized IS NULL OR normalized = '' THEN
    RAISE EXCEPTION 'lifting level required';
  END IF;

  IF normalized NOT IN ('new', 'beginner', 'intermediate', 'advanced', 'veteran') THEN
    RAISE EXCEPTION 'invalid lifting level';
  END IF;

  UPDATE public.profiles
  SET lifting_level = normalized,
      updated_at = now()
  WHERE id = auth.uid();
...
DECLARE
  v_profile record;
  v_schedule jsonb;
  v_split_enabled boolean := true;
  v_split_row_found boolean := false;
  v_goals text[];
  v_name text;
  v_lifting_level text;
  v_today_key text;
  v_tomorrow_key text;
  v_today_label text;
...
BEGIN
  SELECT id, first_name, last_name, username, goals, age, weight, height, lifting_level
  INTO v_profile
  ...
  v_lifting_level := COALESCE(NULLIF(trim(v_profile.lifting_level), ''), 'new');
  ...
  INTO v_split_enabled, v_schedule
  FROM public.split_information
  WHERE user_id = p_user_id;

  v_split_row_found := FOUND;

  IF NOT v_split_row_found OR NOT COALESCE(v_split_enabled, true) THEN
  ...
  ELSE
    v_schedule := COALESCE(v_schedule, public._default_split_schedule());
  ...
  v_context := jsonb_build_object(
    ...
    'athlete', jsonb_build_object(
      ...
      'lifting_level', v_lifting_level,
      'training_years', CASE v_lifting_level
        WHEN 'new' THEN '0 years'
        WHEN 'beginner' THEN '1-2 years'
        WHEN 'intermediate' THEN '3-4 years'
        WHEN 'advanced' THEN '5-6 years'
        WHEN 'veteran' THEN '7+ years'
        ELSE '0 years'
      END
    ),
    'split', v_split,
